...
const included = [
  ...
  'Access to Seedlift’s curated early adopter network',
        ...
        <div className="mx-auto mt-12 max-w-2xl rounded-[2rem] border border-blue-400/30 bg-gradient-to-b from-blue-500/14 to-panel p-2 shadow-glow">
          <div className="rounded-[1.55rem] border border-white/10 bg-ink/92 p-8 sm:p-10">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-3 text-slate-400">
                  A focused 14-day grassroots acquisition sprint for early-stage B2C apps.
                </p>
              </div>

              <span className="w-fit rounded-full bg-success/15 px-3 py-1 text-xs font-semibold text-emerald-300">
                Flat fee
              </span>
            </div>
            ...
            <p className="mt-4 text-sm leading-6 text-slate-400">
              After checkout, you’ll complete a short intake so we can understand your app,
              target user, launch link, and tester requirements before starting your sprint.
            </p>

            <p className="mt-3 text-xs leading-5 text-slate-500">
              If we don’t think Seedlift can reasonably help your product, we’ll refund you
              before starting the sprint.
            </p>

            <div className="mt-6 rounded-2xl border border-line bg-white/5 p-4 text-sm leading-6 text-slate-400">
              Optional after the sprint:{' '}
              <span className="font-semibold text-white">
                $99/week ongoing community seeding
              </span>{' '}
              for founders who want the same channels kept warm.
            </div>
          </div>
        </div>
      </div>
